...
from assignment3.cs231n.layer_utils import *
...
def init_three_layer_convnet(input_shape=(3, 32, 32), num_classes=10,
  ...
  model['W2'] = np.random.randn(int(H * W * F1 / 4), FC)
...
def three_layer_convnet(X, model, y=None, reg=0.0, dropout=None):
  ...
  conv_param = {'stride': 1, 'pad': int((W1.shape[2] - 1) / 2)}
...
def supercool2_convnet(X, model, y=None, reg=1e-4, return_probs=False, compute_dX=False, extract_features=False):
  # 1 conv_relu_forward
