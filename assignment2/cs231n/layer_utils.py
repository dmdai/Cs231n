...
def conv_bn_relu_pool_forward(x, w, b, gamma, beta, conv_param, pool_param, bn_param):
  a, conv_cache = conv_forward_fast(x, w, b, conv_param)
  an, bn_cache = spatial_batchnorm_forward(a, gamma, beta, bn_param)
  ar, relu_cache = relu_forward(an)
  out, pool_cache = max_pool_forward_fast(ar, pool_param)
...
def conv_bn_relu_pool_backward(dout, cache):
  ...
  dma = max_pool_backward_fast(dout, pool_cache)
  dan = relu_backward(dma, relu_cache)
  da, dgamma, dbeta = spatial_batchnorm_backward(dan, bn_cache)
  dx, dw, db = conv_backward_fast(da, conv_cache)
  return dx, dw, db, dgamma, dbeta
